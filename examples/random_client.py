...
async def run(server: str, board_size: int, seed: int | None, move_delay: float) -> None:
    rng = random.Random(seed)
    uri = f"{server.rstrip('/')}/ws/matchmake?board_size={board_size}"
        ...
        async with websockets.connect(uri) as websocket:
            async for raw in websocket:
                ...
                if message_type == "joined":
                    ...
                    pending_move = False
                elif message_type in {"game_over", "opponent_disconnected", "error"}:
                    return
...
def main() -> None:
    ...
    asyncio.run(run(args.server, args.board_size, args.seed, args.move_delay))
